...
export function ProductCard({ product = {}, onAdd, style = {} }) {
  const { name = 'Product', brand = '', price = 0, image, condition = 'new', rating, reviews, stock, icon = 'smartphone' } = product;
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', padding: 'var(--pad-card)' }}>
        {brand && <span style={{ fontSize: 'var(--fs-xs)', fontWeight: 'var(--fw-semibold)', letterSpacing: 'var(--ls-wide)', textTransform: 'uppercase', color: 'var(--text-muted)' }}>{brand}</span>}
        <span style={{ fontSize: 'var(--fs-lg)', fontWeight: 'var(--fw-semibold)', color: 'var(--text-strong)', lineHeight: 'var(--lh-snug)' }}>{name}</span>
        {rating != null && <Rating value={rating} count={reviews} size={14} />}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '4px' }}>
          <span style={{ fontSize: 'var(--fs-h4)', fontWeight: 'var(--fw-bold)', color: 'var(--text-strong)' }}>{usd(price)}</span>
        </div>
