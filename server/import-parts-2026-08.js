...
const ITEMS_FILE = path.join(__dirname, '_tmp_new_items_by_image.json');
...
async function findOrCreateChild(parentId, name) {
  ...
  let doc;
  try {
    doc = await new Category({ name: name.trim(), parent: parentId || null }).save();
  } catch (err) {
    if (err.code !== 11000) throw err;
    // A differently-punctuated name (e.g. "iPhone SE (2022)" vs "iPhone SE
    // 2022") slugified to the same value as an existing sibling — reuse it.
    const slug = name.trim().toLowerCase().replace(/[^a-z0-9\s-]/g, '').replace(/\s+/g, '-');
    doc = await Category.findOne({ parent: parentId || null, slug });
    if (!doc) throw err;
  }
